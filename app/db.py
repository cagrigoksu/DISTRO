...
SEEDS = {
    ...
    "connections": ["USB-C", "Lightning", "Micro-USB", "Other"],
    "statuses": ["Good", "To recycle"],
    "donor_types": ["Company", "Donor", "Recycling Center", "Mobile Bag"],
}
...
def get_or_create(conn, table, name, extra_columns=None):
    name = (name or "").strip()
    if not name:
        return None
...
def get_or_create_brand(conn, name):
    return get_or_create(conn, "brands", name)

def get_or_create_model(conn, brand_id, name):
    name = (name or "").strip()
    row = conn.execute(
    ...
    if row:
        return row["id"]
    conn.execute("INSERT INTO models(brand_id,name) VALUES (?,?)", (brand_id, name))
    return conn.execute("SELECT last_insert_rowid()").fetchone()[0]
